...
{
    private class WebPMemoryOwner(nint handle, int size) : MemoryManager<byte>
    {
        ...
        protected override void Dispose(bool disposing)
        {
            if (_handle != 0)
            {
                UnsafeNativeMethods.WebPFree(_handle);
                _handle = 0;
            }
        }
        ...
        {
            return new Span<byte>((byte*)_handle, size);
        }

        public override unsafe MemoryHandle Pin(int elementIndex = 0)
        {
            return new MemoryHandle((byte*)_handle + size);
        }
    ...
    // public unsafe void GetInfo(byte[] rawWebP, out int width, out int height, out bool has_alpha, out bool has_animation, out string format)
    // {
    ...
    //         result = UnsafeNativeMethods.WebPGetFeatures((IntPtr) ptrRawWebP, rawWebP.Length, ref features);
    //
    //         if (result != 0)
    ...
    //         var size = (int)((long)wpic.custom_ptr - (long)initPtr);
    ...
    // }
    //
    // private unsafe int MyWriter([In] IntPtr data, UIntPtr data_size, ref WebPPicture picture)
    // {
    //     Buffer.MemoryCopy((void*)data, (void*)picture.custom_ptr, (long)data_size, (long)data_size);
    //     picture.custom_ptr += (int)data_size;
    //     return 1;
    // }
    //
